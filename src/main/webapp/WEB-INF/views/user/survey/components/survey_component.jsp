	...
	<c:forEach var="qnEx" items="${surveyQn.QN_EX}" varStatus="qn_status">
		<c:if test="${surveyQn.EX_TYPE eq '일반'}"><!-- 일반 항목 -->
			...
			<c:if test="${qnEx.EX_TYPE eq '텍스트(텍스트)'}">
				<span class="span_font15"><c:if test="${not empty qnEx.EX_TXT1_UNIT}"><c:out value="${qnEx.EX_TXT1_UNIT} : "/></c:if><input type="text" qnTxtLink="<c:out value='${qnEx.QN_CD}-${qnEx.EX_CD}'/>" <c:if test="${qnEx.IS_DISABLE eq 'Y' }">disabled="disabled"</c:if> <c:if test="${not empty qnEx.DISABLE_TARGET}">disableTarget="<c:out value='${qnEx.DISABLE_TARGET }'/>"</c:if> class="<c:out value='${qnEx.EX_CLASS1}'/> input_txt_100" value="<c:out value="${qnEx.ANS_TXT1}"/>" qnCd="<c:out value='${qnEx.QN_CD}'/>" exCd="<c:out value='${qnEx.EX_CD}'/>" qnType="<c:out value='${surveyQn.QN_TYPE}'/>" exType="<c:out value='${qnEx.EX_TYPE}'/>"  />(<input type="text" qnTxtLink="<c:out value='${qnEx.QN_CD}-${qnEx.EX_CD}'/>" <c:if test="${qnEx.IS_DISABLE eq 'Y' }">disabled="disabled"</c:if>  class="<c:out value='${qnEx.EX_CLASS2}'/> input_txt_100" value="<c:out value="${qnEx.ANS_TXT2}"/>" qnTxtLink="<c:out value='${qnEx.QN_CD}-${qnEx.EX_CD}'/>" qnCd="<c:out value='${qnEx.QN_CD}'/>" exCd="<c:out value='${qnEx.EX_CD}'/>" qnType="<c:out value='${surveyQn.QN_TYPE}'/>" /><c:if test="${not empty qnEx.EX_TXT2_UNIT}"><c:out value="${qnEx.EX_TXT2_UNIT}"/></c:if>)</span>
				<br/>
			</c:if>
...
<c:if test="${surveyQn.EX_TYPE eq '표'}"><!-- 표 항목 -->
	...
	<!-- 소아 신규 -->
	<c:if test="${surveyQn.SURVEY_SN eq '2' }">
		<jsp:include page="./survey_table_child_new.jsp" />
	</c:if>
	<!-- 소아 재방문 -->
